...
const Footer = async () => {
  ...
  return (
    <>
      <footer className="border-t border-[#e5d3c6] bg-white py-20 mt-26 text-[#7c5a47]">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex flex-col items-center md:items-start gap-2">
            <span className="text-4xl font-extrabold tracking-tight" style={{ color: '#BA7D62' }}>Elegentlife</span>
            <span className="text-4xs">© {new Date().getFullYear()} Barcha huquqlar himoyalangan.</span>
          </div>
          <div className="flex flex-col md:flex-row gap-4 md:gap-8 text-2sm items-center">
            {categories.map((category: any) => (
              <a
                key={category._id}
                href={`/category/${category.slug.current}`}
                className="hover:text-[#BA7D62] transition-colors"
              >
                {category.title}
              </a>
            ))}
          ...
          <div className="flex gap-4 items-center">
            <a href="#" aria-label="Telegram" className="hover:text-[#BA7D62]">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M9.036 15.956l-.398 4.01c.57 0 .818-.244 1.116-.537l2.675-2.56 5.547 4.05c1.016.56 1.74.266 1.99-.94l3.61-16.84c.33-1.55-.563-2.16-1.56-1.8L1.36 9.17c-1.52.6-1.5 1.46-.26 1.85l4.13 1.29 9.57-6.04c.45-.28.87-.13.53.18"/></svg>
            </a>
